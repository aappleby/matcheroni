...
using expression = Ref<match_expression>;

using exp_mul = Seq<expression, star, expression>;

using exp_add = Seq<expression, plus, expression>;

const token* match_expression(const token* a, const token* b, void* ctx) {
  using expression = Oneof<exp_mul, exp_add, constant>;
...
int test_c99_peg() {
  ...
  token toks[] = {
    TOK_CONST, TOK_STAR, TOK_CONST, TOK_PLUS, TOK_CONST
  };

  auto tok_count = sizeof(toks) / sizeof(toks[0]);
  auto tok_a = toks;
  auto tok_b = tok_a + tok_count;

  auto end = exp_add::match(tok_a, tok_b, nullptr);
  int match_size = end ? end - tok_a : 0;
